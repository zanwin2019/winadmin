...
// 获取树getRoleMenu
const getRoleMenu = () => {
	var params = { schema_name: schema.value }
	useTenantApi().getTenantMenu(params).then(res => {
		state.checkStrict = true
		nextTick(() => {
			treeRef.value?.setCheckedKeys(res.data.map(o => { return o.id }));
		});
	})
}
...
const onSubmit = async () => {
	...
	var data = { ...{ schema_name: schema.value }, ...{ menu_ids: treeData } }
	await useTenantApi().perTenant(data)
	closeDialog();
}
