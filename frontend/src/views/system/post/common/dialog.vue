...
const editId = ref();
const emits = defineEmits(['handleQuery','PosthandleClose']);
...
const openDialog = (type: string, row: EmptyObjectType) => {
	...
	if (type === 'add') {
		...
		state.dialog.title = `修改岗位【${row.name}】`;
...
const onSubmit = async () => {
	...
	if (formType.value === 'edit') {
		...
		await usePostApi().editPost(data, params)
	} else {
		await usePostApi().addPost(state.ruleForm)
	}
	closeDialog();
}
onMounted(async () => {
