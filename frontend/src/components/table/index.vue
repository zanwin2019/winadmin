<template>
	<div class="table-container">
		...
		<el-table
			...
			row-key="id"
			style="width: 100%"
			:header-cell-style="{background:'#f8f8f8',color:'#606266',height:'40px'}"
			:row-style="{height: '40px'}"
			v-loading="config.loading"
			@selection-change="onSelectionChange"
		>
			<el-table-column type="selection" :reserve-selection="true" width="50" v-if="config.isSelection" />
			<el-table-column type="index" align="center" label="序号" width="80" v-if="config.isSerialNo" />
			<el-table-column
			...
			>
				<template v-slot="scope">
					...
					<template v-if="item.slot">
						<slot :name="item.slot" :scopeData="scope"> </slot>
					</template>
					<template v-else>
						{{ scope.row[item.key] }}
					</template>
			...
			<el-table-column label="操作" width="100" v-if="config.isOperate" fixed="right">
				<template v-slot="scope">
		...
		<div class="table-footer mt15">
			...
			<div class="table-footer-tool" v-if="config.isSetup">
